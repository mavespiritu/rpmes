...
class report{
   ...
   $config =  parse_ini_file('../Config/config.ini');
    ...
    , tblprojaccomplishment.q'.$quarter.'submittor as submittor
    ,date(now()) as datenow
FROM
    acknowledgement
    ...
    INNER JOIN tblprojaccomplishment 
        ...
        WHERE acknowledgement.year =:year
        AND
        tblagency.agencyid = :agencyid
        AND
        acknowledgement.quarter = :quarter
        AND
        year(tblprojaccomplishment.q'.$quarter.'datesub) = :year
	GROUP BY agencycode
        ');
...
WHERE tbluser.userid = :userid
      ...
      $statement1->bindParam(':quarter', $quarter);
...
switch ($_GET['quarter']){
    case 1: 
        $quarterInWord = "First Quarter";
        break;
    case 2: 
        $quarterInWord = "Second Quarter";
        break;
    case 3: 
        $quarterInWord = "Third Quarter";
        break;
    case 4: 
        $quarterInWord = "Fourth Quarter";
        break;
}
...
pre{max-width:800px; font-family:Arial, sans-serif;font-size:10px;padding:5px 5px;word-break:normal;}
#footr{
...
<table class="tg">
 ...
 <b>       Subject: </b>
        RPMES Form II: Physical and Financial Accomplishment Report on Agency Action Plan for the Fourth Quarter of CY 2020,
        
        <!--'.$quarterInWord.' of '.$_GET['year'].'., -->
  ...
  <tr >
    ...
    <td class="tg-baqh" >'.$data1['datenow'].'<br>Date</td>
    <td class="tg-baqh" >
