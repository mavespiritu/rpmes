...
<script type="text/javascript" src="./API/javascript/jquery.quicksearch.js"></script>
   ...
   <ul class="nav nav-tabs" role="tablist">
    ...
    <li class="active"><a href="?page=6d7f200fac43f37f5b196bdc0152d9e56541c7f3&Panel=f9f7917226ec6e9fbadd4fe52c35f233baccd628&ack=d7fc60fa1ba1c37c5e4f2dae65e7a9a3b2c79eba"  role="tab" >
            Project Exception <span class="badge"><?php echo $badgeCount->countException($GLOBALS['year']); ?></span> </a></li>
</ul>
    <br/>
    <h2>Acknowledgement for exception</h2>
...
if($agencysmrtbl!=0){
$result = $ack->getByAgencyEXC($year,$agencysmrtbl);  
}
else 
    {

$result = $ack->getAllEXC($year);
}
...
$ctrN = 1;
      ...
      for($ctr24 = 0;$ctr24<$ctr0;$ctr24++){//outer loop
              $result = $ack->getByAgencyEXC($year,$rowrow[$ctr24][0]);  
         ...
         '.$ctrN++.')</b> </td><td>'.$rowrow[$ctr24][2].'      </td>    ';  
              ...
              for($ctr=0;$ctr<4;$ctr++){
               if(isset($row[$ctr])){
                       $subdate = $ack->getdateSubEXC($row[$ctr][0], $row[$ctr][1], $row[$ctr][5]);
   ...
   <a  class="btn '.$status.' " text-align:left;" title="Submission Date: '.$subdate.'"
title="Acknowledge '.$row[$ctr][6].'"
                    data-toggle="modal" 
                    data-target="#acknowledgeexc" 
                    data-idd="'.$row[$ctr][0].'" 
                    data-title="'.$row[$ctr][6].'"
                    data-findings="'.$row[$ctr][2].'"
                    data-action="'.$row[$ctr][3].'"
                    data-quarter="'.$row[$ctr][1].'"
                    data-year='.$row[$ctr][5].'                  
><span class="glyphicon glyphicon-leaf"></span>        
Q '.$row[$ctr][1].'</a>

   <a  class="btn btn-primary"  text-align:left;" title="Submission Date: '.$subdate.'"
title="Print Acknowledgement of '.$row[$ctr][6].'"  
                    data-toggle="modal" 
                    data-target="#acknowledgeOutexc" 
                    data-idd="'.$row[$ctr][0].'" 
                    data-title="'.$row[$ctr][6].'"
                    data-findings="'.$row[$ctr][2].'"
                    data-action="'.$row[$ctr][3].'"
...
><span class="glyphicon glyphicon-print"></span>
 ...
 if($rowB['q1']) echo $rowB['q1']; else echo '<td style="width:20%;" ><a style="width:40%;" class="btn btn-danger"><span class="glyphicon glyphicon-leaf"></span> Q1</td>';
 if($rowB['q2']) echo $rowB['q2']; else echo '<td style="width:20%;" ><a style="width:40%;" class="btn btn-danger"><span class="glyphicon glyphicon-leaf"></span> Q2</td>';
 if($rowB['q3']) echo $rowB['q3']; else echo '<td style="width:20%;" ><a style="width:40%;" class="btn btn-danger"><span class="glyphicon glyphicon-leaf"></span> Q3</td>';
 if($rowB['q4']) echo $rowB['q4']; else echo '<td style="width:20%;" ><a style="width:40%;" class="btn btn-danger"><span class="glyphicon glyphicon-leaf"></span> Q4</td>';
...
<div class="modal fade bs-example-modal-lg" id="acknowledgeOutexc" tabindex="-1" 
     role="dialog" >
...
<div class="modal fade bs-example-modal-lg" id="acknowledgeexc" tabindex="-1" 
     role="dialog" >
  <div class="modal-dialog modal-lg" >
    <div class="modal-content" role="document">
